...
const KEY : any = fs.readFileSync(privateKeyPath, 'utf8');
const KEY_ID : any = process.env.KEY_ID
const ISSUER_ID : any  = process.env.ISSUER_ID
const APP_BUNDLE_ID : any = process.env.APP_BUNDLE_ID;
      ...
      async function onSuccess(validatedData : any) {
        ...
        if (latestReceipts.length > 0) {
          ...
          if (expiresDateMs) {
            ...
            let parentSubscription : any = await SUBSCRIPTION.findOne({ familyId })
      ...
      async function onSuccess(validatedData : any) {
        ...
        if (latestReceipt) {
          ...
          if (expiresDateMs > Date.now()) {
            ...
            const saveWebhookData = await SUBSCRIPTION_WEBHOOK.create(data)
...
export const checkSubscriptionStatus =  async (req : Request , res : Response) => {
  try {
    ...
    let { familyId } = req.query as any;

    if(!familyId) throw new Error('familyId is required.');

    const subscription : any = await SUBSCRIPTION.findOne({ familyId });
...
export const subscriptionStatusCheckAppleSide =  async (req : Request , res : Response) => {
  try {
    ...
    let { originalTransactionId } = req.query as any;
